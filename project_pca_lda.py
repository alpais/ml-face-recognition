...
def create_background_model(images, filename):
  ...
  # limits the number of kept eigenfaces
  ...
  # we make the assumption that there are exactly 10 images per class
  lda_classes = chunks(projections, 10)
  lda_trainer = bob.learn.linear.FisherLDATrainer()
  lda_machine, lda_eigen_values = lda_trainer.train(lda_classes)
  print("Training done.")
  # save the results
...
def enroll(images, background_filename, filename):
  ...
  # loads both the PCA and LDA "background" model
...
def probe(images, background_filename, model_filename):
  ...
  **unlikely** match.
  ...
  # projects the images through both PCA and LDA after linearizing them
  probes = [lda_machine(pca_machine(image.flatten())) for image in images]
