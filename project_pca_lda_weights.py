...
def create_background_model(images, filename):
  ...
  # compute an LDA projection of the retained pca eigen vectors
  projections = [pca_machine(image.flatten()) for image in images]

  # split the training model photos into classes
  lda_classes = chunks(projections, 10)

  # train an LDA machine
  lda_trainer = bob.learn.linear.FisherLDATrainer()
  lda_machine, unused_eigen_values = lda_trainer.train(lda_classes)

  # compute means:
  features = [lda_machine(pca_machine(image.flatten())) for image in images]
  feature_count_lda = (lda_machine(pca_machine(images[0].flatten()))).shape[0]
  feature_means = numpy.mean(numpy.array(features), axis=0)
  feature_sigmas = numpy.std(features, axis = 0)

  pca_machine.save(bob.io.base.HDF5File(filename + "-pca", 'w'))
  lda_machine.save(bob.io.base.HDF5File(filename + "-lda", 'w'))
...
def enroll(images, background_filename, filename):
  ...
  feature_weights = numpy.ones(feature_count_lda)

  # loads our "background" model
  pca_machine = bob.learn.linear.Machine(bob.io.base.HDF5File(background_filename + "-pca", 'r'))
  lda_machine = bob.learn.linear.Machine(bob.io.base.HDF5File(background_filename + "-lda", 'r'))

  # projects the data after linearizing them
  projections = [lda_machine(pca_machine(image.flatten())) for image in images]
  ...
  # solution 1: fixed weights
  s = 2
  theta = 2
  for i in range(0, feature_count_lda):
    # solution 1: update features with fixed weights
...
def probe(images, background_filename, model_filename):
  ...
  **unlikely** match.
  ...
  # loads both pca and lda "background" model
  pca_machine = bob.learn.linear.Machine(bob.io.base.HDF5File(background_filename + "-pca", 'r'))
  lda_machine = bob.learn.linear.Machine(bob.io.base.HDF5File(background_filename + "-lda", 'r'))
  ...
  probes = [feature_weights * lda_machine(pca_machine(image.flatten())) for image in images]
