...
def create_background_model(images, filename):
  ...
  feature_count = settings.variables["pca_components"]
...
def enroll(images, background_filename, filename):
  ...
  for i in range(0, feature_count):
    ...
    if abs(local_means[i] - feature_means[i]) >= feature_sigmas[i] / settings.variables["threshold"] and local_sigmas[i] <= feature_sigmas[i] / settings.variables["threshold"]:
      feature_weights[i] *= settings.variables["boost"]

    # solution 2: 
    # | local variance | global variance | trust in variable |
    # | high           | low             | very low          |
    # | low            | high            | very high         |
    # | high           | high            | no change         |
    # | low            | low             | no change         |
    # formula: global variance / local variance:
    # feature_weights[i] = feature_sigmas[i] / local_sigmas[i]
...
def probe(images, background_filename, model_filename):
  ...
  **unlikely** match.
  ...
  # without counting weights:
  # result1 = [-1.0 * numpy.linalg.norm((model - probe)) for probe in probes]

  # with counting weights
  result2 = [-1.0 * numpy.linalg.norm(feature_weights * (model - probe)) for probe in probes]

  # compare scores:
  # print sum(result1), " vs ", sum(result2)
